...
Now that DNS Privacy can be improved by deploying [@?RFC7858], the clear text
SNI parameter in TLS sticks out.

In the past, there have been multiple attempts at defining SNI encryption.
These attempts have generally floundered, because the simple design fail 
to mitigate several of the attacks listed in {{sni-req}}.
...
* Service: a specific service, such as a web site

* Multiplexed Server: a server that hosts multiple service.

* Service Name: the domain name of the service.

* Hidden Service: the service that the user wants to access without informing 
  on path adversaries.

* Fronting Service: the service that will accept connections on
  behalf of the Hidden Service.



# TLS Encapsulation Protocol
...
# Security Considerations {#sni-req}

## Site Disclosure by Replay Attacks {#replay}
...
The encapsulation protocol proposed in this draft mitigates this attack, because any reply
from the server will be encrypted with a key derived from the session resumption key. The
attackers may replay the connection attempt, but it will not be able to decipher
the server's responses.

## Site Disclosure by Side Channels

The solution mitigates the replay attacks described in the previous section
...
## Leakage of Widely Shared Secrets
...
The TLS encapsulation protocol mitigates this attack, because the
shared secrets are session resumption tokens, which are only valid for a specific client.
...
The TLS encapsulation protocol mitigates this attack, because the
server only needs to recognize the encapsulated Hello.

## Session Filtering {#snireqdontstickout}.
...
The TLS encapsulation protocol partially mitigates this attack. 
The initial messages will contain an encrypted extension, and in this sense do stick out
somewhat. On the other hand, since the extension is encrypted, the adversaries cannot
tell whether the client is using TLS encapsulation or some other encrypted extension.

## Security Context Sharing Attacks
...
The TLS encapsulation protocol mitigates this attack, because the
session keys are negotiated end to end between the client and the 
protected service. 
